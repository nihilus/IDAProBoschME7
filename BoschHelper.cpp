...
//			msg("0x8000 read at 0x%x\n", eaAddr);
		...
		else
		{
			...
			eaLenOfGeneratedCode = ua_code(eaAddr);//create the disassembled code and return the length of it
...
bool BoschHelper::SetC16xRegs(const char *szRegName, sel_t value)
{
	...
	if(SetDefaultRegisterValue(NULL, iReg, value))
		msg(" successful.\n");
	else
	{
		msg(" failed.\n");
...
void BoschHelper::MakeC166Offset(ea_t eaAddr, int nOp)
{
	...
	selSelector = getSR(eaAddr, iReg) << 4;
...
void BoschHelper::MakeDissCode(string sECU)
{
	if(sECU == "bNewME711")
	{
		...
		CreateDissCode(0x80000e, 0x8affff);
	}
...
void BoschHelper::SearchForDTCFlagSetting(string sECU)
{
	...
	set_const_cmt(get_const(enumtID, 0x1, NULL, 0x1),"Select DTC Group A",1);
	add_enum_member(enumtID,"DTCFieldB_H1",	0x2,	0x2);
	set_const_cmt(get_const(enumtID, 0x2, NULL, 0x2),"Select DTC Group B",1);
	add_enum_member(enumtID,"DTCFieldC_H2",	0x4,	0x4);
	set_const_cmt(get_const(enumtID, 0x4, NULL, 0x4),"Select DTC Group C",1);
	add_enum_member(enumtID,"DTCFieldD_H3",	0x8,	0x8);
	set_const_cmt(get_const(enumtID, 0x8, NULL, 0x8),"Select DTC Group D",1);
