...
unsigned char fs_sigs[NUM_FUNC_SIGS][310] = {
			//Jump to R4:R5
			{0xEC, 0x0F5, 0x0EC, 0x0F4, 0x0DB, 0x000},
			...
			0xff, 0xff, 0xf2, 0xfb, 0xff, 0xff, 0x00, 0xa4, 0xdc, 0x5b, 0x98, 0x4a, 0xa8, 0x5a},
...
unsigned int fs_siglen[NUM_FUNC_SIGS] = {6, 62, 104, 98, 58, 252, 108, 202, 64, 40, 28, 44, 58, 120, 150, 66, 96, 
...
unsigned int ts_siglen[NUM_DATA_SIGS] = {128, 256, 36, 88, 36, 64, 60, 60, 64, 38, 38, 16, 16, 16, 40, 40, 20, 20, 16};
...
//	ea1stAddr = find_binary(ea_t startea,ea_t endea,ubinstr,16,sflag);
	...
	for(nIndexA=eaStartAddress; nIndexA < eaEndAddr;)
	{
		...
		{// We're matched for the 1st char,now check the rest
			//msg("FindBinaryWithDontCare() matched 1st char '0x%x' at 0x%x with length %lu\n",nRead, nIndexA, nSLength);
...
ea_t FunctionSigs::CreateFunctionAndComment(ea_t eaAddr, unsigned char* pFuncName, unsigned char* pComment, unsigned int iTries)
{
	...
	if (functFunction!=NULL)
	{
		...
		set_func_cmt(functFunction, (const char*)pComment, TRUE);//Make a repeatable comment
...
void FunctionSigs::Comment(ea_t eaAddr, unsigned char* pDataName, unsigned char* pComment, unsigned int iTries)
{
	...
	{
		char ucBuf[512];
		qsnprintf(ucBuf, 40, "%s_%x", (char*)pDataName, eaAddr);//Create a unique name
...
//		if( generate_big_comment((const char *)pComment, COLOR_AUTOCMT) == 0)
		if(	set_cmt(eaAddr, (const char *)pComment, TRUE) == 1 )// comment the label and make repeatable
		{
...
//for(uIndex=0; uIndex < 0; uIndex++) // For testing
	{
		...
		for(iTry=eaStartAddr, iTries=0; iTry<eaEndAddr; iTry=eaAddr,iTries++)//We may have duplicates
		{
			...
			eaAddr = FindBinaryWithDontCare(fs_sigs[uIndex], fs_siglen[uIndex], iTry, eaEndAddr);
			if (eaAddr != BADADDR)
			{
...
//	eaEndAddr=0xbfb90;
	for(uIndex=0; uIndex < NUM_DATA_SIGS; uIndex++)
	{
		...
		{
			//Search from the last place searched until the end of the database
			eaAddr = FindBinaryWithDontCare(ts_Tablesigs[uIndex], ts_siglen[uIndex], iTry, eaEndAddr);
			if (eaAddr != BADADDR)
			{
